                    ...
                    <div class="x_panel">
                        <div class="x_title">
                            ...
                            <ul class="nav navbar-right panel_toolbox">
                                <a href="{{route('admin.products.courses.create')}}" class="btn btn-info">ساخت دوره جدید</a>
                            </ul>
                            <div class="clearfix"></div>
                        </div>
                        <div class="x_content">
                            <table id="datatable-checkbox" class="table table-striped table-bordered bulk_action">
                                ...
                                @foreach($courses as $course)
                                    <tr>
                                        <td><input type="checkbox" value="{{$course->id}}" class="flat" name="table_records"></td>
                                        <td>{{$course->product->title}}</td>
                                        <td>{{$course->product->price}}</td>
                                        <td>{{$course->product->rate}}</td>
                                        <td>{{0}}</td>
                                        <td>{{$course->jalali_date}}</td>
                                        <td>
                                            <a href="{{route('admin.products.courses.edit',$course->id)}}" class="btn btn-success">ویرایش</a>
                                            <a href="{{route('admin.products.courses.delete',$course->id)}}" class="btn btn-danger">حذف</a>
                                        </td>
                                    </tr>
                                @endforeach
                                </tbody>
